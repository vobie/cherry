...
function noop(input, args, meta) {
  return input;
}

function first(input, args, meta) {
  return input[0];
} // these arrays behaving incredibly weird


function literal(input, args, meta) {
  return args;
}
...
function $$(input, args, meta) {
  return meta.engine.select(args, input);
}
...
function sh(input, args, meta) {
  if (!process.env.ENABLE_UNSAFE_PIPE_CMDS) {
    throw new Error('Disabled for security reasons');
  }

  return _child_process.default.execSync(`echo ${input} | ${args}`).toString();
...
function attr(input, args, meta) {
  return (input[0] || input).attribs[args];
}
...
function text(input, args, meta) {
  function getText(node) {
    ...
    if (node.type === 'text') {
      ...
      return node.children.map(getText).map(_it => {
        return _it.trim();
      }).join(' ');
    } else {
      return '';
