...
const extractor = {
  $: 'table',
  // selecting first table. Multi-select is done
  tableAs2dArr: [{
    $: 'tr',
    _v: ['$ td | text']
  }],
  ...
  showoff: {
    noop: 'noop',
    // same as empty pipeline ("")
    bash: 'sh echo some shit && uptime \\| cut -f9-12 -d" " ',
    // sh commands can be escaped - ! - escaping might be a security weakness, Can preceding cmd escape away the next?
...
console.log(html);
console.log(`Result:`);
console.dir((0, _index.pick)(html, extractor));